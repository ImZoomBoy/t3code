...
/**
 * One named Windows limit. Each limit below is its own export so a skip site
 * reads as the reason it skips, not as a bare platform check.
 */
const windowsLimit = (): boolean => isWindowsHost;
 ...
 */
export const skipPosixShellStub = windowsLimit();
 ...
 */
export const skipBatchStubChildExit = windowsLimit();

/** Windows has no `mkfifo`; its named pipes are a different API and cannot be opened by path. */
export const skipPosixFifo = windowsLimit();
 ...
 */
export const skipUnsafeFileIds = windowsLimit();

...
export const normalizeDispatchCommand = (
  ...
  Effect.gen(function* () {
    ...
    // or set a thread's berth.
    if (
      canonicalCommand.type === "thread.read-only.clear" ||
      canonicalCommand.type === "thread.fleet-berth.set"
    ) {
      return (
    ...
    if (
      canonicalCommand.type === "thread.turn.start" &&
      issuer !== "fleet" &&
      canonicalCommand.bootstrap?.createThread?.readOnly !== undefined
    ) {

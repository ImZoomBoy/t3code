...
describe("normalizeDispatchCommand attachments", () => {
  it.effect("accepts 100 inline images and rejects 101 before writing files", () =>
    Effect.gen(function* () {
      ...
      const accepted = yield* normalizeDispatchCommand(turnStartCommand({ attachments }), "client");
  ...
  it.effect("rejects decoded image overflow before writing it and removes earlier files", () =>
    Effect.gen(function* () {
      ...
      const error = yield* normalizeDispatchCommand(command, "client").pipe(
...
describe("question attachments", () => {
  it.effect("enforces the total response limit and claims duplicate filenames independently", () =>
    Effect.gen(function* () {
      ...
      const failure = yield* normalizeDispatchCommand(command, "client").pipe(Effect.flip);
      ...
      const normalized = yield* normalizeDispatchCommand(accepted, "client");
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const retry = yield* normalizeDispatchCommand(command, "client");

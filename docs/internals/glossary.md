...
| Term                | Meaning                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| ------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Read-only thread    | A thread whose transcript is a window onto work driven elsewhere. `readOnly` is set at creation. Only the fleet can clear it, and only on a fleet-owned second mate thread, with `thread.read-only.clear`. `requireThreadPromptable` refuses turns and reverts on it.                                                                                                                                                                                                                                                                                                                                                 |
...
| Fleet role          | `fleetRole` on a thread (`first-mate`, `second-mate`, `worker`) and its `fleetRepo`. Set at creation. The fleet can set them later on a fleet-owned thread with `thread.fleet-berth.set`, but never to `first-mate`: only `thread.create` makes the First Mate thread. Clients label and place threads by it, never by title.                                                                                                                                                                                                                                                                                         |
| Deferred turn start | A `thread.turn.start` sent with `whenBusy: "queue"` that found its thread busy. The decider holds it until the thread is free after a normal turn end, or drops it with a reason. A thread whose session ended (stopped, interrupted, or in error after a failed turn) is free once no turn is active, so the start runs at once. A runtime error raised mid-turn keeps the turn active. At startup the server starts any an older server left deferred on a free thread. See `deferredTurnStarts.ts`. Not the "queued turn start" of `ThreadSettlementPolicy`, which is a sent user message no turn has adopted yet. |
| Fleet wake          | A `thread.turn.start` sent with `fleetWake: true`, which First Mate uses to wake its own agent. The turn runs as any other. Its user message keeps `fleetWake`, and clients show it as a folded "Fleet notice" line instead of a message the user typed. It does not set the thread's `latestUserMessageAt`, but the decider's queued turn start guard still counts it.                                                                                                                                                                                                                                               |

...
| Deferred turn start | A `thread.turn.start` sent with `whenBusy: "queue"` that found its thread busy. The decider holds it and starts it in the `thread.session.set` that takes the thread idle. It differs from a "queued turn start" in `ThreadSettlementPolicy`, which is a sent user message no turn has adopted yet. |
